...
for test in test_cases:
    ...
    hole_p1_match = re.search('\[([-,0-9]+),([-,0-9]+)\]',
                              test.split('|')[0].split(' ')[0])
    hole_p2_match = re.search('\[([-,0-9]+),([-,0-9]+)\]',
    ...
    for brick in bricks:
        ...
        p1_match = re.search('\[([-,0-9]+),([-,0-9]+),([-,0-9]+)\]',
                             brick_split[1])
        p2_match = re.search('\[([-,0-9]+),([-,0-9]+),([-,0-9]+)\]',
                             brick_split[2])

        p1 = ThreeDimensionalPoint(p1_match.group(1), p1_match.group(2),
                                   p1_match.group(3))
        p2 = ThreeDimensionalPoint(p2_match.group(1), p2_match.group(2),
                                   p2_match.group(3))

        r1 = Rectangle(TwoDimensionalPoint(p1.y, p1.z),
                       TwoDimensionalPoint(p2.y, p2.z))
        r2 = Rectangle(TwoDimensionalPoint(p1.x, p1.y),
                       TwoDimensionalPoint(p2.x, p2.y))
        r3 = Rectangle(TwoDimensionalPoint(p1.x, p1.z),
                       TwoDimensionalPoint(p2.x, p2.z))
